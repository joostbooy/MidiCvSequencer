#ifndef DrumRandomPage_h
#define DrumRandomPage_h

#include "topPage.h"
#include "patternEditRandomPage.h"

namespace DrumRandomPage {
	//Declarations
	void init();
	void enter();
	void exit();
	...
	void onButton(uint8_t id, uint8_t state) {
		...
		if (step >= 0 && state >= 1) {
			if (settings.selected_step_item() != DrumTrack::TRIGGER) {
	...
	void drawLeds() {
		...
		for (int i = 0; i < 16; ++i) {
			color = track.pattern.random_is_enabled(pattern, DrumTrack::StepItem(item), i) ? Matrix::ORANGE : Matrix::BLACK;
	...
	void drawDisplay() {
		...
		// item name
		PatternEditRandomPage::draw_name(DrumTrack::step_item_text(DrumTrack::StepItem(item)));

		// min
		int min_value = track.read_random_min(pattern, DrumTrack::StepItem(item));
		PatternEditRandomPage::draw_min(DrumTrack::step_value_text(DrumTrack::StepItem(item), min_value));

		// max
		int max_value = track.read_random_max(pattern, DrumTrack::StepItem(item));
		PatternEditRandomPage::draw_max(DrumTrack::step_value_text(DrumTrack::StepItem(item), max_value));
	}
